...
// present, appends a second term (tier 10) — see twoStepMissing. `hard`
// widens the operand ranges for tier 10, which otherwise reused tier 7's
// numbers and felt too easy for the top of the ladder.
function missingOperand(tier, nextId, tail = null, hard = false) {
  const op = ["+", "−", "×", "÷"][randInt(0, 3)];
  let a, b;
  if (hard) {
    if (op === "×") { a = randInt(4, 25); b = randInt(4, 20); }
    else if (op === "÷") { b = randInt(4, 20); a = b * randInt(4, 25); }
    else if (op === "+") { a = randInt(50, 300); b = randInt(50, 300); }
    else { a = randInt(100, 400); b = randInt(2, a - 1); }
  } else if (op === "×") { a = randInt(2, 12); b = randInt(2, 12); }
...
// Tier 10: a × b ± c × d — two products to hold in your head at once. Ranges
// widened from the original 3-12/2-9 (products up to 108) so the top tier
// takes real mental effort instead of reusing tier-8-sized arithmetic.
function twoStepDouble(tier, nextId) {
  const id = `m${nextId()}`;
  const outerOp = Math.random() < 0.5 ? "+" : "−";
  let a = randInt(6, 25), b = randInt(6, 20), c = randInt(4, 15), d = randInt(4, 15);
  if (outerOp === "−" && a * b <= c * d) {
    // Swap the pairs so the left product is the larger one; the result stays
    // positive without rejecting and regenerating.
    [a, b, c, d] = [c, d, a, b];
  }
  if (outerOp === "−" && a * b === c * d) d = Math.max(4, d - 1);
...
// Uses missingOperand's `hard` ranges and a wider tail for the same reason.
function twoStepMissing(tier, nextId) {
  const op = Math.random() < 0.5 ? "+" : "−";
  return missingOperand(tier, nextId, { op, c: randInt(10, 60) }, true);
}
...
const GENERATORS = {
  ...
  // Second operand pushed to 11-20 (mul) / quotient 11-20 (div) so tier 4
  // never lands inside tier 2's fully-overlapping 2-9 x 2-10 window — a
  // roll like "5 x 9" was valid at tier 4 but indistinguishable from tier 2.
  4: (nextId) => Math.random() < 0.5 ? mul(2, 12, 11, 20, 4, nextId) : div(2, 12, 11, 20, 4, nextId),
...
const TIER_TIME = { 1: 6, 2: 6, 3: 8, 4: 8, 5: 10, 6: 10, 7: 11, 8: 13, 9: 13, 10: 18 };
