...
function generateItems() {
  ...
  // Full single-digit multiplication tables (2-9 x 2-9) live in tier 1
  // alongside add/sub — otherwise tier 1 stays trivial for too long and
  // the real step-up (two-digit operands, then division) doesn't arrive
  // until far too many rounds in.
  const GENERATORS = {
    1: () => Math.random() < 0.5 ? addSub(1, 20, 1, nextId) : mul(2, 9, 2, 9, 1, nextId),
